...
def index():
    ...
    # Stats
    cursor.execute('SELECT COUNT(*) FROM reports WHERE status="rescued" OR status="adopted"')
    rescued_count = cursor.fetchone()[0]
    
    cursor.execute('SELECT COUNT(*) FROM reports WHERE status="adopted"')
    adopted_count = cursor.fetchone()[0]
    
    cursor.execute('SELECT SUM(amount) FROM donations')
    total_donations = cursor.fetchone()[0] or 0
    
    # Recent animals
    cursor.execute('SELECT * FROM reports WHERE status IN ("pending", "rescued") ORDER BY id DESC LIMIT 6')
...
def report():
    if request.method == 'POST':
        ...
        db = get_db()
        db.execute('INSERT INTO reports (animal_type, location, description, contact, status) VALUES (?, ?, ?, ?, ?)',
...
def adopt():
    ...
    if search_query:
        animals = db.execute('SELECT * FROM reports WHERE status IN ("pending", "rescued") AND (animal_type LIKE ? OR location LIKE ?)', 
                             ('%'+search_query+'%', '%'+search_query+'%')).fetchall()
    else:
        animals = db.execute('SELECT * FROM reports WHERE status IN ("pending", "rescued")').fetchall()
...
def donate():
    if request.method == 'POST':
        ...
        try:
            amount = float(amount)
            db = get_db()
            db.execute('INSERT INTO donations (donor_name, amount) VALUES (?, ?)', (donor_name, amount))
            db.commit()
